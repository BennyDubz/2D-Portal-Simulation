...
# Considers the manhattan distance to the goal from the other side of a portal
def manhattan_including_portals(state, goal):
    if not state[2] or not state[3]:
        return portal_manhattan_heuristic(state, goal) * 1.25
    ...
    return min(best_distance_to_goal * 1.25, portal_manhattan_heuristic(state, goal) * 1.25)
